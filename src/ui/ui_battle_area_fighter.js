...
class UIBattleAreaFighter extends GWE.UIWidget {
  ...
  setCharacter(character) {
    ...
    if (character) {
      ...
      this.uiCharacterSprite.loadFromFile(character.getSpriteFile());
  ...
  isActive() {
    return this.node.classList.contains('UIBattleAreaFighter.u-active');
  }

  setActive(active) {
    this.node.classList.toggle('UIBattleAreaFighter.u-active', active);
  }
  ...
  async handleEffectInflict(data) {
    ...
    this.uiEffectSprite.show();
    this.uiEffectSprite.play(data.effect.getSpriteAnimationName());
    await GWE.eventManager.wait(this.uiEffectSprite, 'E_SPRITE_COMPLETE');
    this.uiEffectSprite.hide();
  ...
  async handleCharacterDecreaseHP(data) {
    ...
    if (this.character.getAttribute('HP') <= 0) {
      ...
      await GWE.eventManager.wait(this.uiCharacterSprite, 'E_SPRITE_COMPLETE');
    }
