...
class BootScreen extends GWE.Screen {
  ...
  onEnter() {
    ...
    this.uiMenu.addWidget(new GWE.UIMenuText({ text: 'Lancer le mode combat' }));
    this.uiMenu.addWidget(new GWE.UIMenuText({ text: 'Menu' }));
    this.uiMenu.addWidget(new GWE.UIMenuText({ text: 'Magasin' }));
  ...
  handleMenuItemSelected(data) {
    if (data.index == 0) {
      ...
      GWE.screenManager.requestSetScreen(new ShopScreen(this.app), { mode: SHOP_SCREEN_MODE.COMMON_STORE, inventoryId: 'inventory_0000' });
    }
  }
}
